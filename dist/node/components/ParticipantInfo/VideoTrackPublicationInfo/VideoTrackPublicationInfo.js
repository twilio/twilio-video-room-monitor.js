...
const IntervalUpdateDatum = useIntervalUpdate_1.withIntervalUpdate(Datum_1.default);
const VideoTrackInfo = ({ track, trackSid }) => {
    const dimensions = useVideoTrackDimensions_1.default(track);
    const isSwitchedOff = useIsTrackSwitchedOff_1.default(track);
    ...
    if (trackData) {
        ...
        const numPacketsLost = packetsLost !== null && packetsLost !== void 0 ? packetsLost : 0;
...
const VideoTrackPublicationInfo = ({ publication }) => {
    const publishPriority = usePublishPriority_1.default(publication);
