...
const RoomStatsProvider = ({ children }) => {
    var _a, _b;
    const previousStatsRef = react_1.useRef();
    const receivedBitrateHistoryRef = react_1.useRef([]);
    const sentBitrateHistoryRef = react_1.useRef([]);
    const room = useRoom_1.default();
    const stats = useGetStats_1.default(room);
    const previousStats = previousStatsRef.current;
    previousStatsRef.current = stats;
    const totalReceivedBitrate = useStatsUtils_1.getTotalBandwidth('bytesReceived', stats, previousStats);
    const newReceivedBytesHistory = receivedBitrateHistoryRef.current.concat({
        x: Date.now(),
        y: totalReceivedBitrate,
    });
    receivedBitrateHistoryRef.current = exports.truncateFront(newReceivedBytesHistory, constants_1.MAX_STAT_HISTORY_LENGTH);
    const totalSentBitrate = useStatsUtils_1.getTotalBandwidth('bytesSent', stats, previousStats);
    const newSentBytesHistory = sentBitrateHistoryRef.current.concat({ x: Date.now(), y: totalSentBitrate });
    sentBitrateHistoryRef.current = exports.truncateFront(newSentBytesHistory, constants_1.MAX_STAT_HISTORY_LENGTH);
